...
if(!empty($_SESSION["tableName"]) && !empty($_POST["intention"]) && !empty($_POST["stId"]) && !empty($_POST["department"]) )
{
	...
	if($t)
	{
	    if($_POST["result"]==1)
		{
		  if(!$mysql->runsql("UPDATE `KxRecruit` SET `".$intention[$_POST["intention"]]."`='0' , `".$fromstId[$_POST["intention"]]."`='".$_SESSION["stId"]."' WHERE `stId`='".$_POST["stId"]."' && `department`='".$_POST["department"]."'"))
			{
				$json["error"]=0;
