...
if(!empty($_SESSION["stId"]))
{
	...
	if(!empty($department))
	{
		...
		*/
		$json["notice"]=$mysql->getData("SELECT `noticeid`,`title`,`description`,`url`,`date` FROM `Notice` WHERE 1 order by `date` desc");
	...
	else
	{
		...
		$json["notice"]=$mysql->getData("SELECT `noticeid`,`title`,`description`,`url`,`date` FROM `Notice` WHERE `noticetype`='Personal' || `noticetype`='External' order by `date` desc");
	}
