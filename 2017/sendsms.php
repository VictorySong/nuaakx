 ...
 * */
 ...
 {
	 if($_SESSION['authcode']==$post['im_code'])
	 {
		 if(!isset($_SESSION['sms_code_time'])||((time()-$_SESSION['sms_code_time'])>600))
		 {
			 $str="";
			for($i=0;$i<6;$i++)
			{
			...
			$_SESSION['sms_code']=$str;
			$_SESSION['sms_code_time']=time();
		 }
		 else if((time()-$_SESSION['sms_code_time'])<600||(time()-$_SESSION['sms_code_time'])==600){
			 $response='{"errno":"4","time":"'.(time()-$_SESSION['sms_code_time']).'"}';
			 die($response);
		 }
	 }
	 else{
...
$username='sshengli';  //用户名

$password_md5='7240835ee7502289c76f5e9bd4cddb8d';  //32位MD5密码加密，不区分大小写 paswd:asdf12345

$apikey='09ddef18819d3e14fc7dede2492db2bd';  //apikey秘钥（请登录 http://m.5c.com.cn 短信平台-->账号管理-->我的信息 中复制apikey）

$mobile=$post['phone'];  //手机号,只发一个号码：13800000001。发多个号码：13800000001,13800000002,...N 。使用半角逗号分隔。

$content='【南航学生科协】您好，您的验证码是:'.$_SESSION['sms_code'].'有效期为十分钟  【美联软通 http://www.5c.com.cn/ 提供支持】';  //要发送的短信内容，特别注意：签名必须设置，网页验证码应用需要加添加【图形识别码】。
